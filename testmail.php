...
if (array_key_exists('to', $_POST)) {
    ...
    if (array_key_exists('to', $_POST) and in_array($_POST['to'], ['contact', 'contact', 'contact'])) {
        $to = $_POST['to'] . '@arohana.co.in';
    } else {
    ...
    if (!$err) {
        $mail = new PHPMailer;
        $mail->isSMTP();
        $mail->Host = 'arohana.co.in';
        $mail->Port = 587;
        $mail->SMTPAuth = true;                               // Enable SMTP authentication
        $mail->Username = 'contact@arohana.co.in';                // SMTP username
        $mail->Password = 'Arohana11!';                  // SMTP password
        $mail->SMTPSecure = 'tls';  
        $mail->CharSet = 'utf-8';
        //It's important not to use the submitter's address as the from address as it's forgery,
        //which will cause your messages to fail SPF checks.
        //Use an address in your own domain as the from address, put the submitter's address in a reply-to
        $mail->setFrom('contact@arohana.co.in', (empty($name) ? 'Contact form' : $name));
        $mail->addAddress($to);
